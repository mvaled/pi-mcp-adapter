...
const MCP_PANEL_KB_DEFS: KeybindingDefinitions = {
  "mcp.panel.navigateUp":     { defaultKeys: ["up", "ctrl+p"],          description: "Navigate up" },
  "mcp.panel.navigateDown":   { defaultKeys: ["down", "ctrl+n"],        description: "Navigate down" },
...
type PanelKeybindingManager = {
  matches(data: string, action: PanelAction): boolean;
  getKeys(action: PanelAction): string[];
};

type RawKeybindingConfig = Record<string, string | string[] | null | undefined>;

type PanelKeybindingSource = {
  getKeys?: (action: string) => string[];
  getUserBindings?: () => RawKeybindingConfig;
  getResolvedBindings?: () => RawKeybindingConfig;
};

function normalizeKeys(keys: string | string[] | null | undefined): string[] {
...
function createPanelKeybindingsManager(
  definitions: KeybindingDefinitions,
  source: PanelKeybindingSource | null | undefined,
): PanelKeybindingManager {
  const userBindings = typeof source?.getUserBindings === "function" ? source.getUserBindings() : {};
  const resolvedBindings = typeof source?.getResolvedBindings === "function" ? source.getResolvedBindings() : {};

  const resolved = new Map<PanelAction, string[]>();

  for (const [id, def] of Object.entries(definitions)) {
    const action = id as PanelAction;
    const defaults = normalizeKeys(Array.isArray(def.defaultKeys) ? def.defaultKeys : [def.defaultKeys]);

    let keys: string[] = [];

    if (typeof source?.getKeys === "function") {
      try {
        keys = normalizeKeys(source.getKeys(id));
      } catch {
        keys = [];
      }
    }
    ...
    if (keys.length === 0) {
      const configured = userBindings[id] ?? resolvedBindings[id];
      keys = normalizeKeys(configured);
    }
    ...
    resolved.set(action, keys);
  }

  return {
    matches(data: string, action: PanelAction) {
      const keys = resolved.get(action) ?? [];
      return keys.some((key) => matchesKey(data, key));
    },
    getKeys(action: PanelAction) {
...
function panelMatches(kb: PanelKeybindingManager, data: string, action: PanelAction): boolean {
  return kb.matches(data, action);
}
function panelKeys(kb: PanelKeybindingManager, action: PanelAction): string[] {
  return kb.getKeys(action);
}
...
class McpPanel {
  ...
  private keybindings: PanelKeybindingManager;
  ...
  constructor(
    ...
    piKeybindings: PanelKeybindingSource | null | undefined,
    ...
    this.keybindings = createPanelKeybindingsManager(MCP_PANEL_KB_DEFS, piKeybindings);
  ...
  handleInput(data: string): void {
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.quit")) {
      this.cleanup();
      this.done({ cancelled: true, changes: new Map() });
      return;
    }

    if (panelMatches(this.keybindings, data, "mcp.panel.save")) {
      this.cleanup();
    ...
    if (this.descSearchActive) {
      if (panelMatches(this.keybindings, data, "mcp.panel.cancel") || panelMatches(this.keybindings, data, "mcp.panel.confirm")) {
      ...
      if (panelMatches(this.keybindings, data, "mcp.panel.deleteBack")) {
      ...
      if (panelMatches(this.keybindings, data, "mcp.panel.navigateUp")) { this.moveCursor(-1); return; }
      if (panelMatches(this.keybindings, data, "mcp.panel.navigateDown")) { this.moveCursor(1); return; }
      if (panelMatches(this.keybindings, data, "mcp.panel.toggle")) {
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.cancel")) {
      if (this.nameQuery) {
        this.nameQuery = "";
        this.rebuildVisibleItems();
        this.cursorIndex = Math.min(this.cursorIndex, Math.max(0, this.visibleItems.length - 1));
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.confirm")) {
      const item = this.visibleItems[this.cursorIndex];
      if (!item) return;
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.pause")) {
      const item = this.visibleItems[this.cursorIndex];
      if (!item) return;
      const server = this.servers[item.serverIndex];
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.reconnect")) {
      const item = this.visibleItems[this.cursorIndex];
      if (!item) return;
      const server = this.servers[item.serverIndex];
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.descSearch")) {
  ...
  private handleDiscardInput(data: string): void {
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.cancel") || panelMatches(this.keybindings, data, "mcp.panel.discardCancel")) {
    ...
    if (panelMatches(this.keybindings, data, "mcp.panel.discardConfirm")) {
      this.cleanup();
      this.done({ cancelled: true, changes: new Map() });
      return;
    }
    if (panelMatches(this.keybindings, data, "mcp.panel.discardSwitch")) {
  ...
  render(width: number): string[] {
    ...
    lines.push(emptyRow());
    const upKey       = fmtKey(panelKeys(this.keybindings, "mcp.panel.navigateUp")[0]   ?? "up");
    const dnKey       = fmtKey(panelKeys(this.keybindings, "mcp.panel.navigateDown")[0] ?? "down");
    const toggleKey   = fmtKey(panelKeys(this.keybindings, "mcp.panel.toggle")[0]       ?? "space");
    const confirmKey  = fmtKey(panelKeys(this.keybindings, "mcp.panel.confirm")[0]      ?? "return");
    const reconnKey   = fmtKey(panelKeys(this.keybindings, "mcp.panel.reconnect")[0]    ?? "ctrl+r");
    const pauseKey    = fmtKey(panelKeys(this.keybindings, "mcp.panel.pause")[0]        ?? "ctrl+x");
    const dsearchKey  = fmtKey(panelKeys(this.keybindings, "mcp.panel.descSearch")[0]   ?? "?");
    const saveKey     = fmtKey(panelKeys(this.keybindings, "mcp.panel.save")[0]         ?? "ctrl+s");
    const cancelKey   = fmtKey(panelKeys(this.keybindings, "mcp.panel.cancel")[0]       ?? "escape");
    const quitKey     = fmtKey(panelKeys(this.keybindings, "mcp.panel.quit")[0]         ?? "ctrl+c");
