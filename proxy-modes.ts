...
export async function executePause(state: McpExtensionState, serverName: string): Promise<ProxyToolResult> {
  ...
  if (getPausedServers(state).has(serverName)) {
  ...
  getPausedServers(state).add(serverName);
...
export async function executeResume(state: McpExtensionState, serverName: string): Promise<ProxyToolResult> {
  ...
  getPausedServers(state).delete(serverName);
...
function getPausedServers(state: McpExtensionState): Set<string> {
  const s = state as McpExtensionState & { pausedServers?: Set<string> };
  if (!s.pausedServers) {
    s.pausedServers = new Set<string>();
  }
  return s.pausedServers;
}
...
export function executeStatus(state: McpExtensionState): ProxyToolResult {
  ...
  for (const name of Object.keys(state.config.mcpServers)) {
    const isPaused = getPausedServers(state).has(name);
...
export function executeSearch(
  ...
  for (const [serverName, metadata] of state.toolMetadata.entries()) {
    ...
    if (getPausedServers(state).has(serverName)) continue; // tools invisible while paused
...
export function executeList(state: McpExtensionState, server: string): ProxyToolResult {
  ...
  if (getPausedServers(state).has(server)) {
...
export async function executeCall(
  ...
  if (serverName) {
    ...
    for (const [server, metadata] of state.toolMetadata.entries()) {
      if (getPausedServers(state).has(server)) continue; // tools invisible while paused
      const found = findToolByName(metadata, toolName);
      if (found) {
        serverName = server;
        toolMeta = found;
  ...
  if (!serverName && !toolMeta && prefixMode !== "none") {
    const candidates = Object.keys(state.config.mcpServers)
      .filter(name => !getPausedServers(state).has(name)) // skip paused servers
